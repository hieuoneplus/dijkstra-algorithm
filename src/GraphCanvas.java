...
// depending on the arrow direction, follow the x, or the y
        ...
        boolean follow_x = Math.abs(endp[node1][node2].x - startp[node1][node2].x) >
...
// the corresponding weight
        if (follow_x) {
            ...
            } else arrow[node1][node2].x = Math.min(x, hbound);
            arrow[node1][node2].y =
...
// do the same if you follow y
            ...
            } else arrow[node1][node2].y = Math.min(y, hbound);
            arrow[node1][node2].x =
...
// draw arrow
        g.drawLine(startp[i][j].x, startp[i][j].y, endp[i][j].x, endp[i]
                [j].y);
...
// write weight of arrow at an appropriate position
    ...
    public void detailsDijkstraArray2D(Graphics g, int i, int j) {
    ...
    public void endstepDijkstraArray2D(Graphics g) {
        ...
        for (int i = 0; i < numnodes; i++)
            if ((node[i].x > 0) && (dist[i] != -1)) {
                String str = "" + dist[i];
                g.drawString(str, node[i].x - (int) fmetrics.stringWidth(str) / 2
                                - 1,
                        node[i].y + h);
        ...
        // if there where more canditates explain why this node is taken
            ...
            int newpaths = 0;
            for (int i = 0; i < numnodes; i++)
                if ((node[i].x > 0) && (weight[i][minend] > 0) && (finaldist[i]
                        == -1))
                    newpaths++;
            if (newpaths > 0)
            ...
            for (int i = 0; i < numnodes; i++)
                if ((node[i].x > 0) && (finaldist[i] == -1) && (weight[i]
                        [minend] > 0))
                    morenodes = true;
        ...
        showstring = showstring + "Node " + intToString(minend) +
                " will be colored green to indicate " + mindist +
    ...
    public void detailsalg(Graphics g, int i, int j) {
        ...
        if (algorithm == DIJKSTRA)
            detailsDijkstraArray2D(g, i, j);
    }

    public void endstepalg(Graphics g) {
        // more algorithms can be added later
        if (algorithm == DIJKSTRA)
            endstepDijkstraArray2D(g);
...
// draw a new arrow upto current mouse position
        if (newarrow)
            g.drawLine(node[node1].x, node[node1].y, thispoint.x,
                    thispoint.y);
// draw all arrows
        for (int i = 0; i < numnodes; i++)
            for (int j = 0; j < numnodes; j++)
